...
let declare_field env
    (`Field (_, (atd_field_name, kind, annots), atd_ty)) cpp_ty =
  let field_name = get_cpp_field_name atd_field_name annots in
  let opt_default =
    match kind with
  ...
  sprintf "  %s %s%s;" cpp_ty field_name opt_set_default
...
and trans_sum my_name env (_, vars, _) =
  ...
  let cases = List.map (function
    | Atd.Ast.Variant (_, (atd_name, an), opt_ty) ->
        ...
        let cpp_name = get_cpp_variant_name atd_name an in
        let opt_java_ty =
          opt_ty |> Option.map (fun ty ->
            let java_ty = trans_inner env ty in
            (ty, java_ty)
          ) in
        (json_name, cpp_name, opt_java_ty)
  ...
  fprintf out "enum class %s\n{\n"
    class_name;

	List.iter (fun (json_name, cpp_name, opt_ty) ->
		match opt_ty with
		| None ->
				fprintf out "  %s,\n" cpp_name
		| Some (atd_ty, cpp_ty) ->
  ...
  let (java_tys, env) = List.fold_left
      (fun (java_tys, env) -> function
         | `Field (_, (field_name, _, annots), atd_ty) ->
             let field_name = get_cpp_field_name field_name annots in
...
and trans_alias name env annots ty =
  let cpp_name = Atdcpp_names.get_cpp_variant_name name annots in
  fprintf env.output "\ntype %s = %s\n" cpp_name (trans_inner env ty);
  env
  ...
  | Name (_, (_, name1, _), _) ->
      (match norm_ty env atd_ty with
       | Name (_, (_, name2, _), _) ->
           ...
           Atdcpp_names.to_class_name name2
       | _ ->
           Atdcpp_names.to_class_name name1
      )
  | List (_, sub_atd_ty, _)  ->
