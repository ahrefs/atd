...
let alias_wrapper_definition env name type_expr = 
  let cpp_struct_name = struct_name env name in
  [
      Line (sprintf "namespace %s {" cpp_struct_name);
      Block [
...
let type_decl env ((loc, (name, param, an), e) : A.type_def) : B.t =
  if param <> [] then
    not_implemented loc "parametrized type";
  let unwrap e =
    match e with
...
let to_header_file ~atd_filename ~head (items : A.module_body) dst_path =
  let env = init_env () in
...
let to_cpp_file ~atd_filename (items : A.module_body) dst_path =
  ...
  let namespace_end = [Line "} // namespace atd"] in
  Line (fixed_size_preamble atd_filename) :: head @ cpp_defs @ namespace_end
  |> double_spaced
  |> Indent.to_file ~indent:4 dst_path
...
let run_file src_path =
  ...
  in
  to_header_file ~atd_filename:src_name ~head atd_module hpp_dst_path;
  to_cpp_file ~atd_filename:src_name atd_module cpp_dst_path
