...
let preamble ~atd_filename ~user_docstring = [
  Line (sprintf {|"""Generated by atdpy from type definitions in %s.|}
          atd_filename);
  Line "";
  Line (sprintf
...
methods and functions to convert data from/to JSON."
          atd_filename);
  Inline (blank_lines_before user_docstring);
  Inline user_docstring;
  Line {|"""|};
  Line {|
...
let trans_any_doc_to_docstring ~paragraph_width loc an : node list =
  match Atd.Doc.get_doc loc an with
  | None -> []
  | Some blocks ->
      Codegen_doc.make_unquoted_multiline_docstring
...
let trans_alias_doc_to_docstring loc an =
  trans_any_doc_to_docstring
    ~paragraph_width:class_doc_paragraph_width
     loc an
...
let trans_head_doc_to_docstring loc an =
  trans_any_doc_to_docstring
    ~paragraph_width:78
    loc an
...
let alias_wrapper env ~class_decorators ~class_doc name type_expr =
  ...
  let type_doc = trans_alias_doc_to_docstring loc an in
  [
    Inline class_decorators;
    Line (sprintf "class %s:" py_class_name);
    Block [
