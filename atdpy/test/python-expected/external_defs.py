"""Generated by atdpy from type definitions in external_defs.atd.

This implements classes for the types defined in 'external_defs.atd', providing
...
class A:
    ...
    @classmethod
    def from_json(cls, x: Any) -> 'A':
        return cls((lambda x: (_atd_read_int(x[0]), _atd_read_int(x[1])) if isinstance(x, list) and len(x) == 2 else _atd_bad_json('array of length 2', x))(x))

    def to_json(self) -> Any:
        return (lambda x: [_atd_write_int(x[0]), _atd_write_int(x[1])] if isinstance(x, tuple) and len(x) == 2 else _atd_bad_python('tuple of length 2', x))(self.value)

    @classmethod
    def from_json_string(cls, x: str) -> 'A':
