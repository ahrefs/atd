...
def test_imported_types() -> None:
    """Test import with annotation on alias (class_=def_) and on path (if_)."""
    a_obj = e.Tagged(
        ext_tag=if_.Tag("hello"),
        priority=def_.Priority(5),
    )
    a_str = a_obj.to_json_string()
    assert a_str == '{"ext_tag": "hello", "priority": 5}'

    b_obj = e.Tagged.from_json_string(a_str)
    assert b_obj.ext_tag.value == "hello"
